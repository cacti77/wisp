...
class BinnedDataTest extends FunSuite with Matchers  {
  ...
  test("Binned list should have one item per bucket") {
    val bd: BinnedData = (new Highcharts()).binIterableNumBins(List(1, 2, 3, 4), 4)
  ...
  test("Empty buckets should receive a count of zero") {
    val bd: BinnedData = (new Highcharts()).binIterableNumBins(List(1, 2, 4), 4)
    bd.toBinned().toArray should be (Array(("1.00 - 2.00", 1), ("2.00 - 3.00", 1), ("3.00 - 4.00", 0), ("4.00 - 5.00", 1)))
  }

  test("Triplet should be binned") {
    val bd: BinnedData = (new Highcharts()).mkTrueTriplet(List((1, 2, 4), (2, 3, 5)))
  ...
  test("SAT scores should create the proper x-axis categories") {
    ...
    val hc = highcharts.histogram(highcharts.binIterable(List(490, 499, 459, 575, 575, 513, 382, 525, 510, 542, 368, 564, 509, 530, 485, 521, 495, 526, 474, 500, 441,
